...
object PlaySounds {
    ...
    fun listRemoteFiles(): List<RemoteSoundFile> {
        val response = createChatBotService().getHtml().execute()
    ...
    fun downloadFile(file: RemoteSoundFile) {
        val response = createNuulsService().get(file.url.removePrefix(HOST_NUULS)).execute()
        val responseBody = response.body()
        if (!response.isSuccessful || responseBody == null) {
    ...
    private interface NuulsService {
        @GET("{name}")
        fun get(@retrofit2.http.Path("name") name: String): Call<ResponseBody>
    }
}
